...
TEST(TxnArgsParserTest, new_order) {
  std::unique_ptr<Parser> parser = std::make_unique<YSQLParser>(
...
TEST(TxnArgsParserTest, payment) {
  std::unique_ptr<Parser> parser =
      std::make_unique<YSQLParser>(TEST_FILE_PATH + "payment_txn.txt", nullptr);
  auto s = parser->Init();
  LOG_INFO << s.ToString();
  EXPECT_EQ(s.ok(), true);
  std::unique_ptr<Txn> t = nullptr;
...
TEST(TxnArgsParserTest, stock_level) {
  std::unique_ptr<Parser> parser =
      std::make_unique<YSQLParser>(TEST_FILE_PATH + "stock_level.txt", nullptr);
  auto s = parser->Init();
  LOG_INFO << s.ToString();
  EXPECT_EQ(s.ok(), true);
  std::unique_ptr<Txn> t = nullptr;
