      ...
      while (retryCount < MAX_RETRY_COUNT) {
        try {
          ...
          std::string UpdateCustomerSQL = format(
              "UPDATE Customer SET C_BALANCE = C_BALANCE - %s, C_YTD_PAYMENT = C_YTD_PAYMENT + %s, 
              C_PAYMENT_CNT = C_PAYMENT_CNT + 1 WHERE C_W_ID =
                                  % s AND C_D_ID = % s AND C_ID =
                                                       % s ",
                                                       std::to_string(payment),
              std::to_string(payment), std::to_string(w_id),
              std::to_string(d_id), std::to_string(c_id));
          txn.exec(UpdateCustomerSQL);
          std::string getCustomerSQL = format(
              "SELECT * FROM Customer WHERE C_W_ID = %s AND C_D_ID = %s AND "
              "C_ID = %s",
              std::to_string(w_id), std::to_string(d_id), std::to_string(c_id));
