...
Status YCQLOrderStatusTxn::Execute(double* diff_t) noexcept {
  ...
  auto st = Retry(std::bind(&YCQLOrderStatusTxn::executeLocal, this), MAX_RETRY_ATTEMPTS);
...
Status YCQLOrderStatusTxn::executeLocal() noexcept {
  ...
  std::cout << format("\t1.C_NAME: (%s, %s, %s), C_BALANCE: %lf",
                                ...
                                GetValueFromCassRow<std::int64_t>(customer_it, "c_balance") / 100.0) << std::endl;
  ...
  std::cout << format("\t2.O_ID: %d, O_ENTRY_D: %lld, O_CARRIER_ID: %d",
                                o_id,
                                GetValueFromCassRow<int64_t>(order_it, "o_entry_d"),
                                GetValueFromCassRow<int32_t>(order_it, "o_carrier_id")) << std::endl;
  ...
  while (cass_iterator_next(orderLine_it)) {
    std::cout << format("\t\tOL_I_ID: %d, OL_SUPPLY_W_ID: %d, OL_QUANTITY: %lf, OL_AMOUNT: %d, OL_DELIVER_D: %lld",
                                  GetValueFromCassRow<int32_t>(orderLine_it, "ol_i_id"),
                                  GetValueFromCassRow<int32_t>(orderLine_it, "ol_supply_w_id"),
                                  GetValueFromCassRow<int64_t>(orderLine_it, "ol_quantity") / 100.0,
                                  GetValueFromCassRow<int32_t>(orderLine_it, "ol_amount"),
                                  GetValueFromCassRow<int64_t>(orderLine_it, "ol_delivery_d")) << std::endl;
  }
...
std::pair<Status, CassIterator*> YCQLOrderStatusTxn::getCustomerInfo() noexcept {
  std::string stmt =
...
std::pair<Status, CassIterator*> YCQLOrderStatusTxn::getOrderLines(int32_t o_id) noexcept {
  std::string stmt =
