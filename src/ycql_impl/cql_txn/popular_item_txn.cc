...
Status YCQLPopularItemTxn::Execute(double* diff_t) noexcept {
  ...
  auto st = Retry(std::bind(&YCQLPopularItemTxn::executeLocal, this), MAX_RETRY_ATTEMPTS);
...
Status YCQLPopularItemTxn::executeLocal() noexcept {
  ...
  while (cass_iterator_next(order_it)) {
    ...
    while (cass_iterator_next(orderLine_it)) {
      ...
      std::cout << format("\t\t(i).Quantity ordered: %s", ol_quantity) << std::endl;
    }
  ...
  for (const auto &[i_name, cnt] : popularItems) {
    ...
    std::cout << format("\t\t(i).Item name: %s", i_name.c_str()) << std::endl;
    std::cout << format("\t\t(ii).Percentage in orders: %lf", percentage) << std::endl;
  }
...
std::pair<Status, CassIterator*> YCQLPopularItemTxn::getMaxOrderLines(int32_t o_id) noexcept {
  std::string stmt =
...
std::pair<Status, CassIterator*> YCQLPopularItemTxn::getLastOrders(int32_t next_o_id) noexcept {
  std::string stmt =
...
std::pair<Status, CassIterator*> YCQLPopularItemTxn::getCustomerName(int32_t c_id) noexcept {
  std::string stmt =
...
std::pair<Status, CassIterator*> YCQLPopularItemTxn::getItemName(int32_t i_id) noexcept {
  std::string stmt =
      "SELECT i_name "
      "FROM item "
      "WHERE i_id = ? "
