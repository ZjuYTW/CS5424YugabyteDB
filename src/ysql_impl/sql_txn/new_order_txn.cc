  ...
  while (retryCount < MAX_RETRY_COUNT) {
    try {
      ...
      for (int i = 0; i < orders_.size(); i++) {
        ...
        SQL_Update_S_Quantity(w_ids[i], i_ids[i], adjust_qty, quantities[i], 1, remote_cnt, &txn);
...
void YSQLNewOrderTxn::SQL_Update_S_Quantity(int w_id, int i_id, int s_quantity, int quantity, int order_cnt, int remote_cnt, pqxx::work* txn) {
  std::string query = format(
      "UPDATE Stock SET S_QUANTITY = %d, S_YTD = S_YTD + %d, S_ORDER_CNT = S_ORDER_CNT + %d, S_REMOTE_CNT = S_REMOTE_CNT + %d WHERE S_W_ID = %d AND S_I_ID = %d",
...
float YSQLNewOrderTxn::SQL_Get_C_Discount(int w_id, int d_id, int id, pqxx::work* txn) {
  pqxx::result res;
