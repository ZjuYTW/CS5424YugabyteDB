  ...
  for (int d_id=1;d_id<=10;d_id++){
    ...
    while (retryCount < MAX_RETRY_COUNT) {
      try {
        ...
        std::string UpdateOrderLine = format("UPDATE orderline SET ol_delivery_d = '%s' WHERE ol_w_id = %d  AND ol_d_id = %d AND ol_o_id = %s ",
        ...
        std::string GetSumQuery = format("SELECT sum(ol_amount) as sumVal FROM orderline WHERE ol_w_id = %d AND ol_d_id = %d AND ol_o_id = %s",
                                         w_id_,d_id,order_id);
        ...
        //get customer id
        std::string GetCustomerIdQuery = format("SELECT o_c_id FROM orders WHERE o_w_id = %d AND o_d_id = %d AND o_id = %s",
                                         w_id_,d_id,order_id);
        ...
        std::string UpdateCustomer = format("UPDATE customer SET c_balance = c_balance + %s, c_delivery_cnt = c_delivery_cnt + 1 WHERE c_w_id = %d AND c_d_id = %d AND c_id = %d",
